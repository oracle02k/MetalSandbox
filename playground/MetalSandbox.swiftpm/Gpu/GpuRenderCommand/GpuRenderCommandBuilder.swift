...
class GpuRenderCommandBuilder {
    ...
    private let allocator: GpuTransientAllocator
    private let renderCommandRepository: GpuRenderCommandRepository
    private let functions: GpuFunctions
    private let renderStateResolver: GpuRenderStateResolver
    private let tileShaderParams = GpuTileShaderParams()
    ...
    init(
        allocator: GpuTransientAllocator,
        renderCommandRepository: GpuRenderCommandRepository,
        functions: GpuFunctions,
        renderStateResolver: GpuRenderStateResolver
    ) {
    ...
    func makeDispatchParams() -> GpuRenderCommandDispatchParams{
        return GpuRenderCommandDispatchParams(
            commandBuffer: renderCommandRepository.currentBuffer(), 
            tileShaderParams: tileShaderParams
        )
    }
    
    func withStateScope(_ body: (GpuRenderCommandBuilder) -> Void) {
        descriptors.push(RenderDescriptors())
        body(self)
        descriptors.pop()
    }
    ...
    func findFunction(by name: GpuFunctions.FunctionTable) -> MTLFunction {
    ...
    func allocFrameHeapBlock<T>(_ type: T.Type, length: Int = 1) -> GpuTransientHeapBlock {
        return allocator.allocate(size: MemoryLayout<T>.stride * length)
    }
}

extension GpuRenderCommandBuilder {
    enum BindType {
        case vertex
        case fragment
        
        func setBufferCommand(_ buffer: MTLBuffer, offset: Int, index: Int) -> GpuRenderCommand {
            return switch self {
        ...
        func setOffsetCommand(_ offset: Int, index: Int) -> GpuRenderCommand {
            return switch self {
    ...
    func setVertexBuffer<T>(_ value: T, index: Int) {
        let allocation = allocator.allocate(size: MemoryLayout<T>.stride)
    ...
    func setVertexBuffer<T>(_ value: [T], index: Int) {
        let allocation = allocator.allocate(size: value.byteLength)
