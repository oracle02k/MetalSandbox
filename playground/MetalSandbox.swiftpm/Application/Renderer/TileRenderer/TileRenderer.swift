...
class TileRenderer
{
    ...
    var supportsOrderIndependentTransparency = false
    var enableOrderIndependentTransparency = false
    var enableRotation = false
    
    func build(gpu: GpuContext, mtkView:MTKView) {
        loadResources(gpu:gpu)
        loadMetal(gpu:gpu, mtkView: mtkView )
    }
    
    /// Initializes the app's starting values, and creates actors and constant buffers.
    func loadResources(gpu: GpuContext)
    {
        ...
        for i in 0..<MaxBuffersInFlight {
            ...
            let actorParamsBuffer = gpu.makeTypedBuffer(options: .storageModeShared) as TypedBuffer<ActorParams>
    ...
    func loadMetal(gpu: GpuContext, mtkView:MTKView)
    {
        ...
        do {
            ...
            renderPipelineDesc.colorAttachments[RenderTargetIndices.Color.rawValue].pixelFormat = mtkView.colorPixelFormat;
        ...
        if supportsOrderIndependentTransparency {
            ...
            do {
                ...
                tileDesc.colorAttachments[RenderTargetIndices.Color.rawValue].pixelFormat = mtkView.colorPixelFormat;
    ...
    func updateState() {
        ...
        var cameraParams = cameraParamsBuffers[currentBufferIndex].contents
        cameraParams.viewProjectionMatrix = matrix_multiply(projectionMatrix, viewMatrix);
    ...
    func drawOpaqueObjects(renderEncoder:MTLRenderCommandEncoder, renderPipelineState:MTLRenderPipelineState) {
        ...
        for actorIndex in 0..<opaqueActors.count {
            let offsetValue = actorIndex * Align(sizeof(ActorParams.self), BufferOffsetAlign)
    ...
    func drawTransparentObjects(renderEncoder:MTLRenderCommandEncoder, renderPipelineState:MTLRenderPipelineState) {
        ...
        for actorIndex in 0..<transparentActors.count {
            let offsetValue = (actorIndex + opaqueActors.count) * Align(sizeof(ActorParams.self), BufferOffsetAlign)
            renderEncoder.setVertexBufferOffset(offsetValue, index:BufferIndices.ActorParams.rawValue)
            renderEncoder.setFragmentBufferOffset(offsetValue,index:BufferIndices.ActorParams.rawValue)
            renderEncoder.drawPrimitives(type: .triangle, vertexStart:0, vertexCount:6)
        }
    ...
    func bindCommonActorBuffers(renderEncoder:MTLRenderCommandEncoder) {
    ...
    func drawWithOrderIndependentTransparency(mtkView:MTKView, commandBuffer:MTLCommandBuffer) {
        guard let drawable = mtkView.currentDrawable?.texture else {
            return
        }
        
        forwardRenderPassDescriptor.colorAttachments[RenderTargetIndices.Color.rawValue].texture = drawable;
        forwardRenderPassDescriptor.depthAttachment.texture = mtkView.depthStencilTexture;
        ...
        bindCommonActorBuffers(renderEncoder: renderEncoder)
    ...
    func drawUnorderedAlphaBlending(mtkView:MTKView, commandBuffer:MTLCommandBuffer) {
        guard let drawable = mtkView.currentDrawable?.texture else {
            return
        }
        ...
        bindCommonActorBuffers(renderEncoder: renderEncoder)
        drawOpaqueObjects(renderEncoder: renderEncoder, renderPipelineState: opaquePipeline)
        //drawTransparentObjects(renderEncoder: renderEncoder, renderPipeline: transparencyPipeline)
    ...
    func drawInMTKView(view:MTKView) {
        ...
        if enableOrderIndependentTransparency {
            drawWithOrderIndependentTransparency(mtkView:view, commandBuffer:commandBuffer)
        } else {
            drawUnorderedAlphaBlending(mtkView:view,commandBuffer:commandBuffer)
        }
