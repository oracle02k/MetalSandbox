...
class IndirectScene: SandboxScene {
    ...
    func gearSetup() {
        ...
        objectParameters = gpu.sharedAllocator.allocateTypedBuffer(
    ...
    func indirectSetup(device: MTLDevice) {
        ...
        indirectFrameStateBuffer = gpu.privateAllocator.allocate(of: FrameState.self)
    ...
    func makeFrameRenderPassNodes(descriptor: MTLRenderPassDescriptor, pixelFormats: AttachmentPixelFormats) -> GpuPassNodeGroup {
        //BlitPass
        let prePassNode = { [self] in
            let frameState = gpu.frame.sharedAllocator.allocateTypedPointer(of: FrameState.self) { p in
        ...
        let mainPassNode = GpuPassNode(
            GpuRenderCommandDispatchPass(descriptor, dispatch: gpu.frame.buildFrameRenderCommand{ builder in
                builder.pixelFormats = pixelFormats
                draw(builder)
            }),
            dependencies: [prePassNode]
        )
        
        return GpuPassNodeGroup(nodes: [mainPassNode, prePassNode], outputNodes: [mainPassNode])
    }
    
    func draw(_ renderCommandBuilder: GpuRenderCommandBuilder) {
    ...
    func normalDraw(_ builder: GpuRenderCommandBuilder) {
    ...
    func indirectDraw(_ builder: GpuRenderCommandBuilder) {
    ...
    func newGearMeshWithNumTeeth(_ numTeeth: Int) -> GpuTypedTransientHeapBlock<Vertex> {
        ...
        return gpu.sharedAllocator.allocateTypedBuffer(length: numVertices) { meshVertices in
